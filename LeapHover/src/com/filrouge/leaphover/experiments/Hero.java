...
public class Hero {
	...
	public void render() {
		...
		Vector2 bottom = new Vector2(startOfRay).add((float) (this.boardWidth/2*Math.cos(angle)*-1), 
													(float) (this.boardWidth/2*Math.sin(angle)*-1));
		
		Vector2 endOfRayFront = new Vector2(front.x,-5);
		Vector2 endOfRayBottom = new Vector2(bottom.x,-5);
		...
		body.getWorld().rayCast(callbackBack, bottom, endOfRayBottom);
